...
ElastiCache For Redis 를 활용해서 제한 수량이 설정된 판매 / 재고량 UPDATE시 분산락 기법을 활용하여 이슈없이 처리할 수 있다.   
...
### ProductService.java ###
분산락 관리를 위한 %d-lock 객체(상품 아이별로 생성) 와, Key 값이 상품아이디이고 Value 가 판매수량인 K/V 객체들이 redis 캐시에 생성되고 관리되어 진다. 이준 분산락을 관리하는 %d-lock 객체들은 휘발성 객체로 tryLock() 함수 호출시 생성되고 unlock() 호출시 소멸된다.    
소스코드 중 lock.tryLock(1, 3, TimeUnit.SECONDS)) 부분의 동작방식은 쓰레드가 락을 획득하기 위해서 1초간 락 획득 시도를 내부적으로 여러번 반복한다는 것을 의미한다. 이떄 3이라는 숫자는 락 획득 후 해당 쓰레드가 락을 해제하지 않는 다면 3초 후에 해당 락이 자동으로 해제된다는 것을 의미한다. 
