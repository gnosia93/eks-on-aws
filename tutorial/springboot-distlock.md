...
소스코드 중 lock.tryLock(1, 3, TimeUnit.SECONDS)) 부분의 동작방식은 쓰레드가 락을 획득하기 위해서 1초간 락 획득 시도를 내부적으로 여러번 반복한다는 것을 의미한다. 내부구현을 추적해 보면 Lua 스크립트와 Pub/Sub 구조를 활용하여 락 획득시도를 1초 동안 여러번 반복하는 것을 확인할 수 있다. 여기서 3이라는 숫자는 락 획득 후 해당 쓰레드가 락을 해제하지 않는다면 3초 후에 해당 락이 자동으로 해제된다는 것을 의미한다. 
```
