ADOT(AWS Distro for [Open Telemetry](https://opentelemetry.io/)) collector 로 k8s 메트릭을 수집해서 AMP 로 내보내고, AMG 로 시각화하는 방법을 알아볼 예정이다.
프로메테우스 helm 차트를 이용해서 수집기를 구성할 수는 있으나, 프로메테우스 서버가 메트릭을 저장하기 위해서는 K8S 볼륨을 필요하다. 즉 메트릭을 볼륨(Prometheus TSDB)에 먼저 저장한 후, AMP 로 전송하는 하는 구조로 되어 있다.  
이 워크샵에서는 EKS 클러스터 에 Volume 을 만들지 않는 관계로, ADOT 를 이용하여 중간 저장 단계없이 바로 메트릭을 AMP 로 보낸다. 

cloud9 터미널에서 아래 스크립트를 먼저 실행한다. 
```
